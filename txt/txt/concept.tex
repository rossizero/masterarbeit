...
\subsection*{Definition Mauerwerksverband}\label{concept:mauerwerksverband}
...
So kann ein Mauerwerksverband als Menge von Schichten, die wiederum aus einer Menge von Bausteintransformationen bestehen, verstanden werden.
...
\begin{enumerate}
    ...
    \item Eine skalierbare Rotation \(r = (w, x, y, z)\).
    \item Einen festen translativen Versatz \(\vec{p}_o = {[x, y, z]}^T\).
    \item Einen festen rotierenden Versatz \(r_o = (w, x, y, z)\).
\end{enumerate}
...
Zur endgültigen Platzierung eines Bausteins anhand einer Bausteintransformationen aus einer Schicht eines Mauerwerksverbands wird lediglich noch ein Faktor benötigt, der angibt, die wievielte Wiederholung der Menge an Bausteintransformationen einer Schicht gerade Anwendung findet.
Sei \(B_1 = (\vec{p}, r, \vec{p}_o, r_o) = ({[l, 0, 0]}^T, (0, 0, 0, 0), {[0, 0, 0]}^T, (1, 0, 0, 0))\) eine Bausteintransformationen, \(M = (l, b, h) = (2, 1, 0.5)\) ein Modul und \(r\) ein Skalar,
dann ergibt sich die Position \(P\) eines Bausteins zur Wiederholung \(r = 1\) wie folgt: \(P = \vec{p}_o + r * \vec{p} = {[0, 0, 0]}^T + 1 * {[2, 0, 0]}^T = {[2, 0, 0]}^T\).
Im nächsten Wiederholungsschritt mit \(r = 2\) ergibt dies eine Position \(P = {[4, 0, 0]}^T\).
Mit \(B_2 = ({[l, 0, 0]}^T, (0, 0, 0, 0), {[\frac{l}{2}, 0, 0]}^T, (1, 0, 0, 0))\) und je einer Schicht gebildet aus den Mengen \(\{B_1\}\) bzw. \(\{B_2\}\) erhält man eine komplette Beschreibung für einen Läuferverband mit einem Versatz von 50\% der Steinlänge.
...
\paragraph*{Ecken, T-Kreuzungen und X-Kreuzungen}
...
Es liegt deshalb nahe, die Eck- und Kreuzungslegepläne zur Beschreibung des Mauerwerksverbands hinzuzufügen, wie es schon im Abschnitt~\ref{Eck- und Kreuzungslegepläne} definiert wurde.
...
So entsteht aus Fall 3 aus Abbildung~\ref{fig:concept:t_joint_variationen} der Fall Nummer 1 und Fall Nummer 2 aus Abbildung~\ref{fig:concept:x_joint_variationen} wird ebenfalls zu Fall 1.
...
So bleibt die Tragfähigkeit der Wand trotz Öffnungen bestehen.
