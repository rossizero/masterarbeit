...
class SmallTestToCompareIFC(Scenario):
    def get_walls(self):
        ...
        w1 = Wall.make_wall(1.0, width, 1.20, np.array([0.9, 5.2, 0.12]),
                             quaternion.from_euler_angles(0, 0, 0 * math.pi),
                             ifc_wall_type="test", name="w0", base_module=base_module, bond_type=bond_type)
        w2 = Wall.make_wall(1.0, width, 1.20, np.array([0.2, 5.5, 0.12]),
                             quaternion.from_euler_angles(0, 0, -0.5 * math.pi),
                             ifc_wall_type="test", name="w0", base_module=base_module, bond_type=bond_type)

        w1_small = Wall.make_wall(1.0, 0.2, 0.12, np.array([0.7, 1.1, 0.12]),
                            quaternion.from_euler_angles(0, 0, 0 * math.pi),
                            ifc_wall_type="test", name="w0", base_module=base_module, bond_type="StretchedBond")
        w2_small = Wall.make_wall(1.0, 0.2, 0.12, np.array([0.1, 1.5, 0.12]),
                            quaternion.from_euler_angles(0, 0, 1.5 * math.pi),
                            ifc_wall_type="test", name="w0", base_module=base_module, bond_type="StretchedBond")

        all = [w2, w1]#, w1_small, w2_small]
        #for wall in all:
